...
cv::Mat performGrabCut(const cv::Mat &inputImage)
{
   ...
   // cv::Rect rectangle(50, 50, 200, 300);
    cv::Rect rectangle(10, 10, 300, 400);
    // 使用 GrabCut 算法进行图像分割
...
int main(int argc, char **argv)
{
    ...
    for (const rosbag::MessageInstance &m : view)
    {
        ...
        else
        {
            ...
            std::cout << "IMU message details:" << std::endl;
            std::cout << "Header:" << imu->header << std::endl;
            std::cout << "Orientation:" << imu->orientation << std::endl;
            std::cout << "Angular Velocity:" << imu->angular_velocity << std::endl;
            std::cout << "Linear Acceleration:" << imu->linear_acceleration << std::endl;
        ...
        {
            std::cout << "odom stamp:" << odom->header.stamp << std::endl;
            std::cout << "Odometry message details:" << std::endl;
            std::cout << "Header:" << odom->header << std::endl;
            std::cout << "Pose:" << odom->pose << std::endl;
            std::cout << "Twist:" << odom->twist << std::endl;
            //o_bag.write(odom_topic, odom->header.stamp, odom);
        }
